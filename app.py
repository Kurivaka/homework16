...
def one_user(user_id):
    if request.method == 'GET':
        ...
        db.session.commit()
        db.session.close()
        return f"Обьект с id {user_id} успешно удален"


@app.route('/orders', methods = ['GET', 'POST'])
def orders():
    if request.method == 'GET':
        return jsonify([order.to_dict() for order in Order.query.all()]), 200
    if request.method == 'POST':
        try:
...
def offers():
    if request.method == 'GET':
        return jsonify([offer.to_dict() for offer in Offer.query.all()]), 200
    if request.method == 'POST':
        try:
